...
import { fetchMe, changePassword as apiChangePassword, logoutUser } from '@/lib/api';
import { useAdminTheme } from '@/context/AdminThemeContext';

export default function SchoolTeacherProfileView() {
  ...
  const passwordMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['schoolTeacherProfile'] });
    },
    onError: (err: unknown) => {
  ...
  const email = profile?.email || `${profile?.username || 'schoolteacher'}@madrasa.org`;
          ...
          <div className="mt-5 w-full flex flex-col gap-2.5 pt-4 border-t border-gray-100 dark:border-gray-700 text-left">
            <div className="flex items-center gap-3 rounded-2xl bg-gray-50 dark:bg-gray-700/50 p-3">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400">Role & Scope</span>
                <span className="text-xs font-semibold text-gray-800 dark:text-gray-200">Secular Subject Instructor</span>
              </div>
            </div>
        ...
        <div className="rounded-3xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden divide-y divide-gray-100 dark:divide-gray-700/60 shadow-xs">
          ...
          <div className="p-4 flex items-center justify-between">
            ...
            <button
              ...
              onClick={() => {
                toggleDarkMode();
                showNotification(!isDarkActive ? 'Dark mode enabled' : 'Light mode enabled');
              }}
              className={`relative inline-flex h-7 w-12 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                isDarkActive ? 'bg-madrasa-700' : 'bg-gray-200'
              }`}
            >
              <span
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">Sign out from school teacher account</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 p-4 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="w-full max-w-sm rounded-3xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-xl text-center">
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              Are you sure you want to sign out from your school teacher account?
            </p>
